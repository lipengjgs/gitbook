...
1. CURL for Windows [下载](https://curl.haxx.se/download.html)

> 将`bin`目录添加到 path 环境变量即可

```s
> curl -r 0-10 -i http:127.0.0.1:5566/index.js
...
Content-Length: 10
Date: Mon, 03 Aug 2020 04:07:39 GMT
Connection: keep-alive
```

2. [win10 kms激活工具](https://github.com/IoTcat/kms)

## 常见[命令](https://docs.microsoft.com/zh-cn/windows-server/administration/windows-commands/windows-commands)

```sh
# ping 向对方主机发送默认大小为32字节的数据，参数：“-l[空格]数据包大小”；“-n发送数据次数”；“-t”指一直ping。
...
# telnet
telnet towel.blinkenlights.nl # 看星球大战
telnet ip 端口 # 远程登陆服务器,默认端口为23
telnet 在本机上直接键入telnet 将进入本机的telnet

# 更换cmd字体颜色 设置cmd控制台前景和背景颜色；0=黑、1=蓝、2=绿、3=浅绿、4=红、5=紫、6=黄、7=白、8=灰、9=淡蓝、A=淡绿、B=淡浅绿、C=淡红、D=淡紫、E=淡黄、F=亮白
color 111111
...
# 退出命令行 用参数/B则是退出当前批处理脚本而不是cmd.exe
exit
...
net user
net user ***              # 查看某个用户信息
net user *** 123456 /add  # 新增用户，并设置密码为123456
net user *** /active:no   # 禁用用户
net user *** /del         # 删除用户
net start # 查看开启了哪些服务
...
del * # -F 文件名 加-F参数后就可删除只读文件,/AR、/AH、/AS、/AA分别表示删除只读、隐藏、系统、存档文件，/A-R、/A-H、/A-S、/A-A表示删除除只读、隐藏、系统、存档以外的文件。例如“DEL/AR *.*”表示删除当前目录下所有只读文件，“DEL/A-S *.*”表示删除当前目录下除系统文件以外的所有文件
      # /S /Q 目录 或用：rmdir /s /Q 目录 /S删除目录及目录下的所有子目录和文件。同时使用参数/Q 可取消删除操作时的系统确认就直接删除。（二个命令作用相同）
...
# 查看文件
dir  # 参数：/Q显示文件及目录属系统哪个用户，/T:C显示文件创建时间，/T:A显示文件上次被访问时间，/T:W上次被修改时间
...
@echo off
set excuteDir=..\..\node_modules\.bin\
set inputProto=..\..\..\doc\hgmj\pb.proto
set outJS=..\..\assets\Hall\Script\Network\pb.js
set outTS=..\..\assets\Hall\Script\Network\pb.d.ts
echo 确保%inputProto%文件已更新！！！
pause

::生产pb文件
%excuteDir%pbjs -t static-module -w commonjs -o %outJS% %inputProto% & %excuteDir%pbts -o %outTS% %outJS% &echo 完成 &pause
```

## 技巧

1. 将多个.ts 文件(视频片段)合成一个文件，前提将 ts 文件顺序命名好：0000，0001，... 1111 ;

    ```s
    copy /b E:\ts\*.ts E:\video\video001.ts
    ```

2. 移动文件命令

    ```s
    move E:\ts\a.ts E:\file
    ```

3. 文件重名名

    ```bat
    ren 1_1.png  card_41.png
    ren 1_2.png  card_42.png
    ren 1_3.png  card_43.png
    ren 1_4.png  card_44.png
    ren 1_5.png  card_45.png
    ren 1_6.png  card_46.png
    ren 1_7.png  card_47.png
    ren 1_8.png  card_48.png
    ren 1_9.png  card_49.png
    ren 1_10.png card_4a.png
    ren 1_11.png card_4b.png
    ren 1_12.png card_4c.png
    ren 1_13.png card_4d.png
...
1. 无法删除某个文件，指定的文件名无效或太长的问题

    > 1、一般可能是文件夹太深，名字太长导致的，可以尝试将文件夹的名称一个个改短即可；
    >
    > 2、进入安全模式进行强行删除；

2. win10防火墙

    > 设置 - 安全 - windows defender - 防火墙

3. hosts 不见

    > 查看 - 选项 - 查看 - 隐藏受保护的操作系统文件

4. 局域网连接

    > 打开【远程桌面连接】，输入局域网地址，输入用户名密码即可，防火墙需要关闭
...
6. github经常访问失败
    ...
    2. 添加附录中的github hosts;

    具体的IP可通过以下网址查：<http://ipaddress.com/>
